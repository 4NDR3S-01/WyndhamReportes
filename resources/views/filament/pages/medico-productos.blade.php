            ...
            <div class="flex flex-col gap-4 2xl:flex-row 2xl:items-center 2xl:justify-between">
                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-emerald-700 ring-1 ring-emerald-100 dark:bg-emerald-950/40 dark:text-emerald-300 dark:ring-emerald-900">
                            Inventario medico
                        </span>
                ...
                <div class="grid w-full gap-3 2xl:max-w-[680px]" style="grid-template-columns: repeat(5, minmax(0, 1fr));">
                    ...
                    <div class="min-w-0 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3 text-center dark:border-emerald-900 dark:bg-emerald-950/20">
                        <p class="truncate text-[10px] font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Medicinas</p>
                        <p class="mt-1 truncate text-2xl font-black tracking-tight text-emerald-700 dark:text-emerald-300">{{ number_format($this->totalMedicinas, 0, ',', '.') }}</p>
                    </div>
                    <div class="min-w-0 rounded-2xl border border-sky-100 bg-sky-50/70 p-3 text-center dark:border-sky-900 dark:bg-sky-950/20">
                        <p class="truncate text-[10px] font-semibold uppercase tracking-wide text-sky-700 dark:text-sky-300">Equipos</p>
                        <p class="mt-1 truncate text-2xl font-black tracking-tight text-sky-700 dark:text-sky-300">{{ number_format($this->totalEquipos, 0, ',', '.') }}</p>
                    </div>
        ...
        <section class="min-w-0 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 sm:px-5">
                <div class="flex flex-col gap-4 2xl:flex-row 2xl:items-end 2xl:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if ($this->porCaducar > 0)
                                <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-100 dark:bg-amber-950/40 dark:text-amber-300 dark:ring-amber-900">
                    ...
                    <div class="flex w-full flex-col gap-2 sm:flex-row sm:items-center 2xl:w-auto 2xl:justify-end" role="search" aria-label="Filtros de productos medicos">
                        ...
                        <div class="flex h-10 w-full items-center gap-2 rounded-xl border border-gray-300 bg-white px-3 shadow-sm transition focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500/20 dark:border-gray-700 dark:bg-gray-950 sm:w-72 xl:w-80">
                        ...
                        <select wire:model.live="tipoFiltro" class="h-10 rounded-xl border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 dark:border-gray-700 dark:bg-gray-950 dark:text-white sm:w-32" style="color-scheme: light dark;">
                        ...
                        <select wire:model.live="estado" class="h-10 rounded-xl border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 dark:border-gray-700 dark:bg-gray-950 dark:text-white sm:w-32" style="color-scheme: light dark;">
            ...
            @if ($this->productos->isEmpty())
                <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                    <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-300">
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @foreach ($this->productos as $p)
                                ...
                                @endphp
                                <tr class="transition hover:bg-emerald-50/40 dark:hover:bg-emerald-950/10">
                                    <td class="px-4 py-3 sm:px-5">
                                        <div class="flex min-w-0 items-start gap-3">
                                            <div class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl {{ $p->tipo === 'equipo' ? 'bg-sky-50 text-sky-700 dark:bg-sky-950/40 dark:text-sky-300' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' }}">
                                                @if ($p->tipo === 'equipo')
                                    ...
                                    <td class="hidden px-4 py-3 lg:table-cell">
                                        @if ($p->fecha_caducidad)
                                            <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $caducaPronto ? 'bg-amber-50 text-amber-700 ring-1 ring-amber-100 dark:bg-amber-950/40 dark:text-amber-300 dark:ring-amber-900' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300' }}">
        ...
        @if ($modalProductoAbierto)
            <div class="flex items-center justify-center p-4 sm:p-6" style="position: fixed; inset: 0; z-index: 99999; background: rgba(15, 23, 42, 0.58); backdrop-filter: blur(6px);" wire:click.self="cerrarModalProducto">
                <section class="w-full overflow-hidden rounded-2xl border border-white/70 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.35)] ring-1 ring-gray-950/5 dark:border-gray-800 dark:bg-gray-900 dark:ring-white/10" style="max-width: 500px;">
                    <div class="bg-gradient-to-br from-emerald-50 via-white to-white px-5 py-4 dark:from-emerald-950/30 dark:via-gray-900 dark:to-gray-900">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex min-w-0 gap-3">
                                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-600 text-white shadow-sm shadow-emerald-600/25">
                    ...
                    <form wire:submit.prevent="guardar" class="space-y-4 px-5 py-4">
                        <div class="grid gap-3 sm:grid-cols-2">
                            <div>
                                ...
                                <select wire:model="tipo" class="mt-1 block w-full rounded-xl border-gray-300 bg-white text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" wire:model="stock_minimo" class="mt-1 block w-full rounded-xl border-gray-300 bg-white text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white">
                            </div>
                        </div>

                        <div>
                            <label class="text-xs font-medium text-gray-600 dark:text-gray-300">Nombre</label>
                            <input wire:model="nombre" placeholder="Ej. Analgan" class="mt-1 block w-full rounded-xl border-gray-300 bg-white text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white" autofocus>
                        ...
                        <div>
                            ...
                            <input type="date" wire:model="fecha_caducidad" class="mt-1 block w-full rounded-xl border-gray-300 bg-white text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white">
                        </div>

                        <div>
                            <label class="text-xs font-medium text-gray-600 dark:text-gray-300">Observaciones</label>
                            <textarea wire:model="observaciones" rows="3" placeholder="Opcional" class="mt-1 block w-full rounded-xl border-gray-300 bg-white text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white"></textarea>
                        </div>

                        <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                            <input type="checkbox" wire:model="activo" class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500"> Disponible para usar
                        </label>
